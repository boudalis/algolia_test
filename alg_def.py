...
def insert_date(df,db_url):
    try:
        ...
        cur.copy_from(output, tgt_table, null="") # null values become ''
        conn.commit()
    except:
...
def display_query(query): # this function return the result of a query in a dataframe 
        try:
            engine = create_engine(db_url) # create sql instance 
            df = pd.read_sql_query(query, con=engine) # run the result of a query in a dataframe
            return df
        except:
            logging.error( "problem with the query or the conenction with the database" )
